...
export interface CountrySelectorProps {
  ...
  disabled?: boolean;
  onToggle: () => void;
  onChange: (code: SelectMenuOption["code"]) => void;
...
export default function CountrySelector({
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (mutableRef.current && !mutableRef.current.contains(target) && open) {
        onToggle();
        setQuery("");
      }
    };
                    ...
                    .map((code, index) => {
                      return (
                        ...
                        >
                          <img
                            alt={`${code.code}`}
                            src={`https://purecatamphetamine.github.io/country-flag-icons/3x2/${code.code}.svg`}
                            className={"inline mr-2 h-4 rounded-sm"}
                          />
